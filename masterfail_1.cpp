...
{ 
	...
	while(1)
	{
		...
		if(isMaster)
		{
		    ...
		    system("ethtool eth0 | grep 'Link detected: no' | wc -l > check.log");
		    std::ifstream check1("check.log");
		    int isEthDown=0;
		    check1>>isEthDown;
		    check1.close();
		    ...
		    system("ethtool eth2 | grep 'Link detected: no' | wc -l > check.log");
			...
			system("./ProcessChecker.sh; echo $? > processcount.log");       
